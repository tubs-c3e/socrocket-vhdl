...
*/
    ...
    gs::gs_param<unsigned int> p_apbctrl_index("hindex", 1u, p_apbctrl); //From 2u to 1u, otherwise it won't work in VHDL, correspondends to hindex of GRLIB
    ...
    gs::gs_param<unsigned int> p_mmu_cache_ilram_size("size", 1u, p_mmu_cache_ilram); // Changed from 0u to 1u -- 0 is not in range of 1 to 512 for VHDL
    gs::gs_param<unsigned int> p_mmu_cache_ilram_start("start", 0x0000008e, p_mmu_cache_ilram); // Changed to ilram start address mentioned in line 552 (0x0000008e)
    gs::gs_param_array p_mmu_cache_dlram("dlram", p_mmu_cache);
    gs::gs_param<bool> p_mmu_cache_dlram_en("en", false, p_mmu_cache_dlram);
    gs::gs_param<unsigned int> p_mmu_cache_dlram_size("size", 1u, p_mmu_cache_dlram); // Changed from 0u to 1u -- 0 is not in range of 1 to 512 for VHDL
    gs::gs_param<unsigned int> p_mmu_cache_dlram_start("start", 0x0000008f, p_mmu_cache_dlram); // Changed to dlram start address mentioned in line 555 (0x0000008f)
    ...
    gs::gs_param<unsigned int> p_apbuart1_index("index", 1, p_apbuart1); // Changed from 9 to 1, otherwise it won't work in VHDL
    gs::gs_param<unsigned int> p_apbuart1_addr("addr", 0x001, p_apbuart1); // Changed from 0x009 to 0x001, otherwise it won't work in VHDL
